/*
  ...
  │  Component   │  Pin   │  Note                                │
  ├──────────────┼────────┼──────────────────────────────────────┤
  │  Green LED   │  26    │  Right side — 220Ω resistor to GND   │
  │  Red LED     │  27    │  Right side — 220Ω resistor to GND   │
  │  Buzzer +    │  25    │  Right side — Active buzzer          │
  │  Motor IN1   │  32    │  Right side — L293D Pin 2            │
  │  Motor IN2   │  33    │  Right side — L293D Pin 7            │
  │  LCD SDA     │  21    │  Left side  — I2C default SDA        │
  │  LCD SCL     │  22    │  Left side  — I2C default SCL        │
  │  GND         │  GND   │  Common GND for all components       │
  └──────────────┴────────┴──────────────────────────────────────┘

  L293D CONNECTIONS:
    Pin 1  (EN1)  → 5V (always enabled)
    Pin 2  (IN1)  → ESP32 GPIO 32
    Pin 3  (OUT1) → Motor terminal A
    Pin 4  (GND)  → Common GND
    Pin 5  (GND)  → Common GND
    Pin 6  (OUT2) → Motor terminal B
    Pin 7  (IN2)  → ESP32 GPIO 33
    Pin 8  (VSS)  → External motor power supply (5V–12V)
    Pin 16 (VCC)  → 5V (ESP32 VIN pin)
  ...
  ALLOWED  → Green LED ON + Motor spins + LCD shows name
  DENIED   → Red LED ON  + Buzzer ON   + LCD shows NOT ALLOWED
  Duration → 5 seconds for all outputs

  Libraries needed (install via Library Manager):
    - LiquidCrystal_I2C by Frank de Brabander
    - ArduinoJson by Benoit Blanchon
    - WiFi, WebServer (built-in with ESP32)
  ─────────────────────────────────────────────────────────────────
*/
...
// ── WiFi ──────────────────────────────────────────────────────
const char* WIFI_SSID     = "Airtel_9500128359_5GHz";
const char* WIFI_PASSWORD = "air74213";
