...
camera_server.py
─────────────────────────────────────────────────────────────────────
Module 2 — runs on PC only.

What it does:
  1. Loads the pre-trained face_model.pkl
  2. Opens the PC webcam
  3. Captures frames continuously
  4. Runs face recognition on each frame
  5. When a face is recognised (or unknown), sends a HTTP POST
     to the Heltec LoRa32 (which is running a tiny web server)
     telling it to display ALLOWED or NOT ALLOWED on its OLED

No ESP32 sends anything to this script.
This script is the one that pushes the result TO the LoRa32.
